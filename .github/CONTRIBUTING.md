...
Schema for the local SQLite state lives in [nengok/state/migrations/](../nengok/state/migrations/) as numbered `NNNN_<name>.sql` files. The store applies any pending migrations inside a single `BEGIN`/`COMMIT` block on every startup and records each one in a `schema_versions` table (`version`, `applied_at`, sha256 `checksum`). Once a migration has been recorded, its file is immutable: editing the SQL changes its sha256, and startup refuses with a message that names the next free version number to use for a follow-up. To add schema, drop a new file `0007_<change>.sql` into the directory and ship it in the same PR as the code that reads the new columns; never rewrite an earlier file.
