...
`pip-audit --skip-editable --strict` gates the dependency tree against known CVEs. `--skip-editable` excludes nengok itself (it is not on PyPI until v0.1 publishes). If a transitive dependency picks up a CVE, the job fails; bump the offending package in `pyproject.toml` rather than suppressing.
