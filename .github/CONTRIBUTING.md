...
`pip-audit --skip-editable` gates the dependency tree against known CVEs. `--skip-editable` excludes nengok itself (it is not on PyPI until v0.1 publishes). The `--strict` flag is omitted on purpose: pip-audit emits the editable skip as a warning, and `--strict` turns warnings into hard failures on Linux runners, which would block every CI run. Real dependency CVEs still exit 1 without `--strict`, so the security gate stays intact. If a transitive dependency picks up a CVE, bump the offending package in `pyproject.toml` rather than suppressing.
