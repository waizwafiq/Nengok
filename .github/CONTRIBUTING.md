...
The canonical reference for every database knob (precedence, dialect matrix, TLS posture, schema isolation, migration commands) is [docs/configuration.md](../docs/configuration.md). The section below is the contributor-facing summary.

By default Nengok keeps cluster and approval state in a SQLite file at `~/.nengok/state.db`. SQLite is the zero-setup default for every command in this guide: no docker, no `DATABASE_URL`, no wizard step. To put state in a database you already run, set `DATABASE_URL` (for example `postgresql+psycopg://user:pass@host/db` or `mysql+pymysql://user:pass@host/db`) and re-run `nengok init`. The TOML key `database_url` accepts the same value if you prefer to keep it off the shell environment, and the env var wins when both are present. Anything outside the four supported dialects (`sqlite`, `postgresql`, `postgresql+psycopg`, `mysql`, `mysql+pymysql`) is rejected at config-load with a `ConfigError` so a typo surfaces before the orchestrator starts.

For developers who want to validate the Postgres or MySQL path locally before pointing at a hosted instance, [deploy/local/docker-compose.postgres.yml](../deploy/local/docker-compose.postgres.yml) and [deploy/local/docker-compose.mysql.yml](../deploy/local/docker-compose.mysql.yml) each bring up one container plus a one-shot `nengok db migrate` sidecar. Both are opt-in; nothing in the default install touches docker. For debugging a hosted Cloud SQL instance from a laptop, [deploy/local/cloud-sql-auth-proxy.sh](../deploy/local/cloud-sql-auth-proxy.sh) and [deploy/local/cloud-sql-auth-proxy.ps1](../deploy/local/cloud-sql-auth-proxy.ps1) download the pinned `cloud-sql-proxy` binary and bind it on `127.0.0.1:5432`. Both scripts read `CLOUD_SQL_INSTANCE_CONNECTION_NAME` from the environment and require `roles/cloudsql.client` and `roles/cloudsql.instanceUser` on the calling principal, plus a one-time `gcloud auth application-default login`.
...
#### Monitoring more than one agent

Set `project_identifiers = ["travel-planner-agent", "qa-agent"]` in `~/.nengok/config.toml` (or `NENGOK_PROJECTS=travel-planner-agent,qa-agent`) and one cycle observes and diagnoses every listed project. Map a runner per project with `agent_runners = { "qa-agent" = "sample_agent.qa_agent.agent:QAAgent" }`; unmapped projects fall back to the shared `agent_runner`. Every runner resolves at startup so a typo in a dotted path fails before the first Phoenix call. Each cluster row records its project, the clusters API and the overview take a `?project=` query parameter, the dashboard list shows a project badge with a project dropdown, and the TUI cluster table gains a Project column. The triage verdict carries a `projects` list so a quiet project costs no observer calls.

Clusters keep their identity across cycles. A recurring failure mode lands in the existing `nengok_clusters` row (exact name match first, then one Flash judge call accepted at `cluster_match_threshold`) instead of minting a fresh id per cycle. Rejected and dismissed clusters re-accrete silently without re-alerting, an approved cluster that recurs escalates with reason `fix_regressed` and fires a notification, and a cluster below `min_cluster_size` stays OPEN until enough members accrete. When two monitored agents fail for the same upstream reason, the cross-agent linker confirms the pair with a judge call and writes a `nengok_cluster_links` row; both cluster detail pages then show an "Also affects" panel, and the TUI grows a Links tab.

Reviewer decisions feed the next cycle. The reject and dismiss flows on all three review surfaces accept an optional clustering tag (`duplicate_cluster`, `mixed_root_causes`, `not_a_failure`); tagged or not, every decision lands in `nengok_cluster_feedback` and the clusterer prompt replays the most recent corrections as short instructions. A wrong machine merge is undone from the cluster detail page: the flagged spans detach and re-enter the next cycle as fresh anomalies. `nengok improve` reads the last 30 days of outcomes, writes a report under `artifacts/improvement/`, and proposes a clustering prompt amendment that a reviewer activates from the overview page; nothing applies without that human decision. With `--dry-run` it also scores the current and the proposed prompt against the labeled set in [golden_dataset/clustering_golden.json](../golden_dataset/clustering_golden.json) (pairwise F1; the live harness gate sits at `clustering_quality_floor`). Set `improve_every_cycles = N` and `nengok watch` runs the retro after every N completed cycles, logging a warning instead of breaking the loop when it fails.
...
Every approve, reject, dismiss, or escalate decision is written to the `approvals` table with the reviewer, free-text reason, a UTC timestamp, and a `source` discriminator that records which surface wrote the row (`dashboard`, `tui`, or `api`). The dashboard cluster detail page renders the full history under the approval card; the API exposes `GET /api/v1/clusters/{id}/approvals` for one cluster (newest first) and `GET /api/v1/approvals?limit=50&before=<approval_id>` for a keyset-paginated cross-cluster feed. `POST /api/v1/clusters/{id}/approvals` records a new decision and is what the dashboard and TUI talk to; the legacy `POST /api/v1/approvals` body shape (`cluster_id`, `decided_by`, `notes`) still works and defaults its `source` to `api`. `nengok export` carries `source` through to both the JSON bundle and the CSV section so compliance reports can group by channel.
...
For operators who run Nengok over SSH and would rather skip the browser, the Textual TUI is an opt-in surface that talks to the same FastAPI server. Install it as `pip install "nengok[tui]"` and launch it next to a running `nengok dashboard --no-browser`. `nengok review --host nengok.internal --port 8080` points at a remote instance over an SSH tunnel; `--auth-token` matches the dashboard's bearer-token contract. The cluster list, detail tabs, and approval modal mirror the React components, and every decision posts to `/api/v1/clusters/{id}/approvals` with `source="tui"` so the audit log can attribute the channel. [docs/tui-review.md](../docs/tui-review.md) carries the install steps and the keybindings table alongside the parity contract. The screen-level smoke tests at [tests/test_tui_screens.py](../tests/test_tui_screens.py) and the cross-surface parity test at [tests/test_tui_approval_parity.py](../tests/test_tui_approval_parity.py) gate the contract on every PR.
